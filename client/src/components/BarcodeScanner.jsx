...
// ZXing hint'leri — agresif tarama + 1D barkodlar dahil bilinen formatlar
...
export default function BarcodeScanner({
  ...
  useEffect(() => {
    ...
    const reader = new BrowserMultiFormatReader(zxingHints, {
      delayBetweenScanAttempts: 100, // daha sık dene (default 500)
    ...
    const start = async () => {
      try {
        // arka kamera + yüksek çözünürlük + sürekli odak
        const constraints = {
          video: {
        ...
        const controls = await reader.decodeFromConstraints(
          ...
          (result, err) => {
            ...
            setLastSeen(text);
            // accept filtresi varsa ve geçmediyse: kullanıcıya görsün ki okundu ama reddedildi
            if (typeof accept === 'function' && !accept(text)) {
  ...
  // Modal'ın altındaki form/parent click handler'larını tetiklememek için
  // doğrudan document.body'e render et + tüm event'leri burada durdur.
  return createPortal(
    <div
      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        {!error && !starting && (
          <div className="absolute bottom-24 left-0 right-0 text-center px-4 pointer-events-none">
            ...
            {rejectCount > 0 && (
              <div className="mt-1 text-[10px] font-mono" style={{ color: '#ffaa55' }}>
                {rejectCount} okuma reddedildi — format uyuşmuyor (IMEI 15 hane olmalı)
              </div>
            )}
          </div>
        )}
