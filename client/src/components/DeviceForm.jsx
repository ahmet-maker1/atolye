  ...
  const submit = async (e) => {
    ...
    try {
      await api.deviceCreate({
        ...
        is_new: undefined, // backend'e gitmesin (sadece UI flag'i)
      });
      onSaved();
    } catch (e) {
      setErr(e.message);
          ...
          <div className="p-3 border" style={{ background: C.paperDeep, borderColor: C.line }}>
            ...
            <div className="grid grid-cols-2 gap-2">
              <label className="flex items-center gap-2 px-3 py-2 cursor-pointer text-sm"
                ...
                <input type="radio" name="is_new" checked={!form.is_new} onChange={() => up('is_new', false)} className="sr-only" />
                İkinci el
              </label>
              <label className="flex items-center gap-2 px-3 py-2 cursor-pointer text-sm"
                style={{
                ...
                <input type="radio" name="is_new" checked={form.is_new} onChange={() => up('is_new', true)} className="sr-only" />
                ⚡ Sıfır cihaz
              </label>
            </div>
            {form.is_new && (
