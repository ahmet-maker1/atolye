...
router.get('/', (req, res) => {
  ...
  // NOT EXISTS clause — silinmiş cihazların hareketlerini hariç tut
  // (manuel hareketlerde device_id NULL — NOT EXISTS true olur, geçer)
  const NOT_DELETED = `
    AND NOT EXISTS (
      SELECT 1 FROM devices d
      WHERE d.id = cash_flow.device_id AND d.deleted_at IS NOT NULL
    )
  `;

  const balance    = db.prepare(`SELECT COALESCE(SUM(CASE WHEN type='in' THEN amount ELSE -amount END),0) AS v FROM cash_flow WHERE 1=1 ${NOT_DELETED}`).get().v;
  const todayIn    = db.prepare(`SELECT COALESCE(SUM(amount),0) AS v FROM cash_flow WHERE type='in' AND date(occurred_at) = date('now') ${NOT_DELETED}`).get().v;
  const todayOut   = db.prepare(`SELECT COALESCE(SUM(amount),0) AS v FROM cash_flow WHERE type='out' AND date(occurred_at) = date('now') ${NOT_DELETED}`).get().v;
  const inventoryValue = db.prepare("SELECT COALESCE(SUM(buy_price + expenses),0) AS v FROM devices WHERE status='stokta' AND deleted_at IS NULL").get().v;

  // Haftalık satış/ciro (son 7 gün) — silinmiş cihazlar hariç
  ...
  // Haftalık kâr — son 7 gün, silinmiş hariç
  const weeklyProfit = db.prepare(`
    SELECT date(sale_date) AS d,
      COALESCE(SUM(sell_price - buy_price - expenses), 0) AS kar
    FROM devices
    WHERE sale_date >= date('now','-6 days') AND status = 'satıldı' AND deleted_at IS NULL
