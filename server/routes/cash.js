...
// GET /api/cash/summary — totals (silinmiş cihazların hareketleri hariç)
router.get('/summary', (req, res) => {
  ...
  const totalIn  = db.prepare(`SELECT COALESCE(SUM(amount),0) AS v FROM cash_flow WHERE type='in'  ${NOT_DEL}`).get().v;
  const totalOut = db.prepare(`SELECT COALESCE(SUM(amount),0) AS v FROM cash_flow WHERE type='out' ${NOT_DEL}`).get().v;
  const todayIn  = db.prepare(`SELECT COALESCE(SUM(amount),0) AS v FROM cash_flow WHERE type='in'  AND date(occurred_at) = date('now') ${NOT_DEL}`).get().v;
  const todayOut = db.prepare(`SELECT COALESCE(SUM(amount),0) AS v FROM cash_flow WHERE type='out' AND date(occurred_at) = date('now') ${NOT_DEL}`).get().v;

  // Last 30 days — silinmiş cihazlar hariç
